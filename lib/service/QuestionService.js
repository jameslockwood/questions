...
QuestionService.prototype.registerVote = function(answerId) {
	...
	if(currentQuestion && currentQuestion.timeLeft > 0) {
		for(var i in this.currentQuestion.answers) {
			...
			if( val.id == answerId ){
				this.currentQuestion.answers[i].voteCount++;
				this.emit('vote', {"answerId": answerId, "voteCount": this.currentQuestion.answers[i].voteCount});
				return;
			}
		}
	}
...
QuestionService.prototype.addQuestion = function(questionText) {
	this.questionQueue.push({
		...
		"answers" : _.clone(this.defaultAnswers)
	});

	console.log(this.questionQueue);

	if(this.questionQueue.length == 1 && !this.currentQuestion){
		this.moveToNextQuestion();
	}
}

QuestionService.prototype.fetchPreviousQuestions = function() {
	//Todo put this in redis so it survives restarts
